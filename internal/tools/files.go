...
func (t *listFilesTool) Execute(ctx context.Context, raw json.RawMessage) (string, error) {
	...
	target, err := safePath(t.root, in.Path)
	if err != nil {
		return "", err
	}
...
func (t *listFilesTool) RetryPolicy() RetryPolicy {
	return RetryPolicy{Retryable: true, MaxAttempts: 3}
}
func (t *readFileTool) RetryPolicy() RetryPolicy { return RetryPolicy{Retryable: true, MaxAttempts: 3} }
