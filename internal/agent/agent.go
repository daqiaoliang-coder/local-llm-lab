	...
	for stepNo := 1; stepNo <= 8; stepNo++ {
		...
		for _, call := range res.ToolCalls {
			...
			if !ok {
				...
				messages = append(messages, openai.ChatCompletionMessage{
					Role: openai.ChatMessageRoleTool, ToolCallID: callID, Content: errMsg,
				})
				continue
			}

			// v0.3：工具的默认策略来自 Tool 的 retry contract。
			// 当前文件工具是只读 deterministic 工具，因此允许恢复重试。
			idempotencyKey := "tool:" + callID
			if err := a.store.CreateToolCall(
				callID, runID, toolStepID, call.Function.Name,
				call.Function.Arguments, idempotencyKey, true, 3,
			); err != nil {
				return a.fail(runID, err)
			}
			...
			if err := tools.ValidateJSON([]byte(call.Function.Arguments)); err != nil {
				...
				messages = append(messages, openai.ChatCompletionMessage{
					Role: openai.ChatMessageRoleTool, ToolCallID: callID, Content: err.Error(),
				})
				continue
			}
			...
			if toolErr != nil {
				...
				messages = append(messages, openai.ChatCompletionMessage{
					Role: openai.ChatMessageRoleTool, ToolCallID: callID, Content: msg,
				})
				continue
			}
			...
			// 每个成功 ToolCall 后记录 checkpoint。
			state := EncodeCheckpointState(map[string]any{
				"run_id": runID,
				"step_id": toolStepID,
				"tool_call_id": callID,
				"messages": messages,
			...
			messages = append(messages, openai.ChatCompletionMessage{
				Role: openai.ChatMessageRoleTool, ToolCallID: callID, Content: result,
			})
		}
